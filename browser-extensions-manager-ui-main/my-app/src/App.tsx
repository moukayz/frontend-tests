...
const MainLogo = ({ className }: { className: string }) => {
  ...
  return (
    ...
    >
      ...
      <path
        fill="currentColor"
        className={`${themeConfig[theme].textColor}`}
...
const FilterButton = ({ name, currentFilter, onClick }: FilterButtonProps) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`rounded-full border border-gray-600 px-4 py-2 transition-colors duration-200 ${isChecked ? `${themeConfig[theme].selectedBackground} ${themeConfig[theme].inverseTextColor}` : `${themeConfig[theme].cardBackground} ${themeConfig[theme].textColor}`}`}
    >
      {name}
    </button>
  );
...
function App() {
  ...
  return (
    <div
      className={`h-screen overflow-y-auto p-10 ${themeConfig[theme].mainBackground} ${themeConfig[theme].textColor}`}
    >
      <div className="flex flex-col px-10">
        ...
        <div className={`${gridCardStyle} pb-4`}>
          <span className="col-start-1 row-start-1 text-2xl font-bold">Search</span>
          <input
            type="text"
            value={searchPattern}
            onChange={handleSearch}
            placeholder="Search"
            className="col-span-full rounded-full border-2 border-gray-400 px-4 py-2 text-black"
          />
        </div>
