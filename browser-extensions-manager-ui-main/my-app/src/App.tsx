...
const FilterButton = ({ name, currentFilter, onClick }: FilterButtonProps) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`rounded-full border border-gray-600 px-4 py-2 transition-colors duration-200 ${isChecked ? `bg-primary-hover text-inverse-foreground` : `bg-primary text-foreground`}`}
    >
      {name}
    </button>
  );
